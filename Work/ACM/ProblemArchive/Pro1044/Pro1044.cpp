...
int T[MAX][MAX];      //记录当前点到起点和终点所花费的时间(用于dfs)  如time[m+1][0]代表终点到起点的时间
    ...
    while(!Q.empty()){
        ...
        for(int i=0;i<4;i++){
            ...
            if(chk(dx, dy)){
                ...
                if(omap[dx][dy]=='@')
					T[s][0]=step[dx][dy];       //当前点到起点的时间
                else if(omap[dx][dy]=='<')
					T[s][m+1]=step[dx][dy];    //当前点到终点的时间
                else if(omap[dx][dy]>='A'&&omap[dx][dy]<='J')
					T[s][omap[dx][dy]-'A'+1]=step[dx][dy];  //当前点到宝石的时间
...
void dfs(int s, int v, int t){
    ...
    for(int i=0;i<=m+1;i++){
        if(vis[i] || T[s][i]==0)continue;
        vis[i] = 1;
        dfs(i, v+val[i], t+T[s][i]);
        vis[i] = 0;
    }
}
int main()
