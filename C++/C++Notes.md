...
## 2.1 C++程序的基本构成
	...
	在编译器编译源程序之前，首先会利用预处理程序对源程序进行预处理。
...
```
	...
	在C++中，用#include命令实现文件包含，其一般形式如下：
```
			#include<文件名>
			#include"文件名"
```
...
C++内联函数
=========
 内联函数能有效避免函数调用的开销，程序执行效率更高。缺点：若被声明为内联函数的函数体非常大，则编译器编译后程序的可执行代码会变得很大。


# 3. C++类与对象 <a id="main-chapter-3"></a>
=========
1.
