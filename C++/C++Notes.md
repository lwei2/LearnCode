...
# 1. <a id="chapter-1"></a>C++简介
    ...
    早期的程序设计被称为结构化程序设计，也叫面向过程的程序设计。
	
	所谓面向对象的程序设计指的是以对象为编程核心的程序设计思想。
	
	类是具有相同属性和行为的一组对象的集合，它为属于该类的全部对象提供了统一的抽象描述。
	
	类是对象的抽象，对象是类的实例化。
	
	在C++中，我们必须利用类来创建对象，而类可以自己定义和使用系统提供的或他人编写的。
	
	C++完全支持面向对象的程序设计,具有封装,抽象,继承,多态特征。
	...
	C++程序开发过程:编写源程序(.cpp),编译程序,连接目标文件,运行程序。
...
```
	...
	在C++中，用#include命令实现文件包含，其一般形式：#include<文件名> 或者#include"文件名"。文件包含命令若使用尖括号表示在系统指定的目录中查询要包含的文件。若使用双引号表示在源文件目录下查找要包含的文件。
...
## 4.4 函数重载
	...
	重载、重写、重定义三者区别：
		1.重载在同一个作用域下，函数名相同，函数的参数不同。
		2.重定义在不同的作用域下，一个基类，一个子类。函数名相同函数体不同的两个方法。若不加作用域会各自调用自己的方法，使得对应的同名方法被隐藏。成员变量也如此。
		3.重写在不同作用域下，子类和父类的函数名、参数、返回值完全相同，父类必须还有virtual关键字（协变除外）
		什么是协变？
