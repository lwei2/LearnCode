...
## 3.2 队列
    ...
    队列的数据集合可以表示为a0,a1,a2,...,an-1，每个数据元素的数据类型为DataType。队列操作集合:1)QueueInitiate(Q)。2）非空否:QueueNotEmpty(Q)。3）入队列:QueueApend(Q,x)。4）出队列:QueueDelete(Q,d)。5）取队头元素:QueueGet(Q,d)。
